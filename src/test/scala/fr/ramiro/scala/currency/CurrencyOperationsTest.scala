...
class CurrencyOperationsTest extends FunSuite {
  private implicit val currencySettings = CurrencySettings(base = GBP, USD -> 2.0, EUR -> 1.5)

  test("implicit conversion from double to amount of money with default currency") {
    val pounds = GBP.apply(2.0)
    val amountWithDefaultCurrency: Amount = 2.0
